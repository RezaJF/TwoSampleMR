...
option_list <- list(
  ...
  make_option("--output", type="character", default="./output",
    help="Output directory of the files formatted for TwoSampleMR, [default='~/output']"),
...
# Format the file for TwoSampleMR
file <- fread(opt$input, sep="\t",header=TRUE,stringsAsFactor=FALSE)
...
write.table(file, paste0(output, "/", opt$Phenotype, ".txt"), sep = "\t", quote=FALSE, col.names = TRUE, row.names = FALSE)

file <- as.data.frame(file, fix.empty.names=FALSE)

file <- format_data(file, type="exposure")
file <- subset(file, file$pval.exposure<0.00000005,)
